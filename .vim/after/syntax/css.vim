" Language:     Colored CSS Color Preview_custom
...
function! s:pow(x, n) "{{{
  ...
  function! s:SetNamedColors(dict) "{{{
    if type(a:dict) != type({})
      call alpaca#print_error('型が違います')
    endif

    for key in keys(a:dict)
      call s:SetNamedColor(key, a:dict[key])
    endfor
  endfunction"}}}
  ...
  if has("gui_running") || &t_Co==256
    ...
    augroup CSSPreview
      autocmd!
      autocmd CursorHold *.css,*.html,*.xhtml,*.xml silent call s:PreviewCSSColorInLine('.')
    augroup END
