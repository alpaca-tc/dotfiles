...
function! s:set_rails_syntax()
  ...
  if path =~# '\v/app/%(channels|controllers|helpers|jobs|mailers|models)/.*\.rb$|/app/views/'
    syn match rubyHelper '\v<%(logger)>[!?:]@!'
  endif

  if path =~# '/app/models/.*\.rb$'
    syn match rubyMacro '\v<%(accepts_nested_attributes_for|attr_readonly|attribute|enum|serialize|store|store_accessor|encrypts|strict_loading_by_default|ignored_columns)>[!?:]@!'
  ...
  if path =~# '/app/helpers/.*_helper\.rb$\|/app/views/'
  ...
  if path =~# '/app/controllers/.*\.rb$\|/app/mailers/.*\.rb$\|/app/models/.*_mailer\.rb$'
  ...
  if path =~# '/app/mailers/.*\.rb$\|/app/models/.*_mailer\.rb$'
  ...
  if path =~# '/app/\w\+/concerns/.*\.rb$'
    syn match rubyMacro '\v<%(included|class_methods)>[!?:]@!'
  endif

  if path =~# '\v/app/%(controllers|helpers|mailers)/.*\.rb$|/app/views/|/test/(controllers|integration|system)/.*_test\.rb$|/spec/(features|requests)/.*_spec\.rb$'
  ...
  if path =~# '/db/migrate/.*\.rb$\|/db/schema\.rb$'
  ...
  if path =~# '/test\%(/\|/.*/\)test_[^\/]*\.rb$\|/test/.*_test\.rb$\|/features/step_definitions/.*\.rb$'
  ...
  if path =~# '/spec/.*_spec\.rb$\|/features/step_definitions/.*\.rb$'
  ...
  if path =~# '\v/test/%(channels|controllers|helpers|integration|mailers|models|system)/.*_test\.rb$'
  ...
  if path =~# '\v/test/%(controllers|integration|system)/.*_test\.rb$'
  ...
  if path =~# '\v/test/%(controllers|helpers|integration|system)/.*_test\.rb$'
  ...
  " endif
  if path =~# '\v/test/%(controllers|integration)/.*_test\.rb$|/spec/%(controllers|requests)/.*_spec\.rb$'
  ...
  if path =~# '/test/system/.*_test\.rb$\|/spec/features/.*_spec\.rb$'
...
if alpaca#is_rails(getcwd())
  call s:set_rails_syntax()
endif
